...
inline std::ostream& operator<<(std::ostream& out, const zedui::Vector3& p) {
  out << "(" << p.x << ", " << p.y << ", " << p.z << ")";
  return out;
}

inline std::ostream& operator<<(std::ostream& out, const zedui::Vector4& p) {
  out << "(" << p.x << ", " << p.y << ", " << p.z << ", " << p.w << ")";
  return out;
}
...
#endif  // ZEDUI_GEOMETRY_VECTOR_H_
