...
//   int result = pthread_attr_setstacksize(&attr,
//   Thread::GetDefaultStackSize()); FML_CHECK(result == 0); result =
//   pthread_create(
//       &thread_, &attr,
//       [](void* arg) -> void* {
//         std::unique_ptr<ThreadFunction> function(
...
void SetThreadName(const std::string& name) {
  ...
  //   // Linux thread names are limited to 16 characters including the
  //   terminating
  //   // null.
...
Thread::Thread(const ThreadConfigSetter& setter, const ThreadConfig& config)
    : joined_(false) {
  zedbase::AutoResetWaitableEvent latch;
  zedbase::RefPtr<zedbase::TaskRunner> runner;

  thread_ = std::make_unique<ThreadHandle>(
      [&latch, &runner, setter, config]() -> void {
        setter(config);
        zedbase::MessageLoop::EnsureInitializedForCurrentThread();
...
zedbase::RefPtr<zedbase::TaskRunner> Thread::GetTaskRunner() const {
  return task_runner_;
}
