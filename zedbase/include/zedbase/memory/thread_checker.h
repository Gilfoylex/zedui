...
class ThreadChecker final {
 ...
 private:
  ...
  //   // Returns true if the current thread is the thread this object was
  //   created
  //   // on and false otherwise.
  //   bool IsCreationThreadCurrent() const {
  //     pthread_t current_thread = pthread_self();
  //     bool is_creation_thread_current = !!pthread_equal(current_thread,
  //     self_); if (disable_next_failure_ && !is_creation_thread_current) {
  //       disable_next_failure_ = false;
  //       return true;
  //     }
  // #ifdef __APPLE__
  ...
  //         FML_DLOG(ERROR) << "Object referenced on a thread other than the
  //         one "
